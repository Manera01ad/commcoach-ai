...
const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  const sendToAntigravity = async (transcript: string): Promise<AssessmentStructuredData | null> => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/antigravity/analyze-session', {
        method: 'POST',
        ...
        callbacks: {
          ...
          onmessage: async (m: LiveServerMessage) => {
            ...
            if (m.serverContent?.inputTranscription) {
              ...
              setInputFeedback(prev => prev + text);
              inputFeedbackRef.current += text;
            }
            
            if (m.serverContent?.outputTranscription) {
              const text = m.serverContent.outputTranscription.text;
              setOutputFeedback(prev => prev + text);
              outputFeedbackRef.current += text;
            }

            if (m.serverContent?.turnComplete) {
              // Commit the voice turn to the UI history
              const userSpeech = inputFeedbackRef.current.trim();
              const aiSpeech = outputFeedbackRef.current.trim();
              
              if (userSpeech) {
                onAddManualMessage('user', userSpeech);
              }
              if (aiSpeech) {
                onAddManualMessage('assistant', aiSpeech);
              }
          ...
          onerror: (e) => {
            ...
            setVoiceError('Connection interrupted. Please try again.');
            stopVoiceSession();
          }
        }
      });
